...
__global__ void findCluster(float* xarray, float* yarray, float* tempx, float* tempy, int* countP, int numPoints, int numClusters, int clusid)
{
 ...
 __shared__ float sx[4*BLOCK_SIZE];
 __shared__ float sy[4*BLOCK_SIZE];
 __shared__ int cp[4*BLOCK_SIZE];
 
 //int i = blockIdx.x * blockDim.x + threadIdx.x;
 int startSection = 4*blockIdx.x*blockDim.x;
 int index1 = startSection + threadIdx.x;
 int index2 = startSection + blockDim.x + threadIdx.x;
 int index3 = startSection + 2*blockDim.x + threadIdx.x;
 int index4 = startSection + 3*blockDim.x +threadIdx.x; 
 
 sx[threadIdx.x] = (index1 < numPoints) ? xarray[index1] : 0.0;
 sx[blockDim.x + threadIdx.x] = (index2 < numPoints) ? xarray[index2] : 0.0;
 sx[2*blockDim.x + threadIdx.x] = (index3 < numPoints) ? xarray[index3] : 0.0;
 sx[3*blockDim.x + threadIdx.x] = (index4 < numPoints) ? xarray[index4] : 0.0;

 sy[threadIdx.x] = (index1 < numPoints) ? yarray[index1] : 0.0;
 sy[blockDim.x + threadIdx.x] = (index2 < numPoints) ? yarray[index2] : 0.0;
 sy[2*blockDim.x + threadIdx.x] = (index3 < numPoints) ? yarray[index3] : 0.0;
 sy[3*blockDim.x + threadIdx.x] = (index4 < numPoints) ? yarray[index4] : 0.0;

 cp[threadIdx.x] = (index1 < numPoints) ? 1 : 0;
 cp[blockDim.x + threadIdx.x] = (index2 < numPoints) ? 1 : 0;
 cp[2*blockDim.x + threadIdx.x] = (index3 < numPoints) ? 1 : 0;
 cp[3*blockDim.x + threadIdx.x] = (index4 < numPoints) ? 1 : 0;
 __syncthreads();
...
//3rds ection
id = 0;
 distMin = (sx[2*blockDim.x + threadIdx.x] - cx_cm[id])*(sx[2*blockDim.x + threadIdx.x] - cx_cm[id]) + (sy[2*blockDim.x + threadIdx.x] - cy_cm[id])*(sy[2*blockDim.x + threadIdx.x] - cy_cm[id]);

 for(int k = 1; k < numClusters; ++k)
 {
  distTemp = (sx[2*blockDim.x + threadIdx.x] - cx_cm[k])*(sx[2*blockDim.x + threadIdx.x] - cx_cm[k]) + (sy[2*blockDim.x + threadIdx.x] - cy_cm[k])*(sy[2*blockDim.x + threadIdx.x] - cy_cm[k]);
   if(distTemp < distMin)
   {
     distMin = distTemp;
     id = k;
 ...
 if (id != clusid)
 {
   sx[2*blockDim.x + threadIdx.x] = 0.0;
   sy[2*blockDim.x + threadIdx.x] = 0.0;
   cp[2*blockDim.x + threadIdx.x] = 0;
 }

//4thsection
id = 0;
 distMin = (sx[3*blockDim.x + threadIdx.x] - cx_cm[id])*(sx[3*blockDim.x + threadIdx.x] - cx_cm[id]) + (sy[3*blockDim.x + threadIdx.x] - cy_cm[id])*(sy[3*blockDim.x + threadIdx.x] - cy_cm[id]);

 for(int k = 1; k < numClusters; ++k)
 {
  distTemp = (sx[3*blockDim.x + threadIdx.x] - cx_cm[k])*(sx[3*blockDim.x + threadIdx.x] - cx_cm[k]) + (sy[3*blockDim.x + threadIdx.x] - cy_cm[k])*(sy[3*blockDim.x + threadIdx.x] - cy_cm[k]);
   if(distTemp < distMin)
   {
     distMin = distTemp;
     id = k;
 ...
 if (id != clusid)
 {
   sx[3*blockDim.x + threadIdx.x] = 0.0;
   sy[3*blockDim.x + threadIdx.x] = 0.0;
   cp[3*blockDim.x + threadIdx.x] = 0;
 }
...
{
 __shared__ float sumx[4*BLOCK_SIZE];
 __shared__ float sumy[4*BLOCK_SIZE];
 __shared__ int sumcount[4*BLOCK_SIZE];
